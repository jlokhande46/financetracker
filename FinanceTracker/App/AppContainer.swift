...
class AppContainer {
    ...
    /// Drains any SMS texts queued by LogBankSMSIntent (locked-phone automations) and
    /// saves them as transactions. Called every time the app enters the foreground.
    ///
    /// Duplicates are suppressed: an SMS already saved within the last 2 minutes is skipped,
    /// matching the same dedup window used by the URL-scheme deep-link path.
    func processPendingSMS() {
        ...
        for text in pending {
            ...
            // Skip if the exact same SMS was already saved in the last 2 minutes.
            ...
            let autoAccount = result.last4.flatMap { last4 in
                accountRepo.fetchAll().first { $0.last4 == last4 }
            }
